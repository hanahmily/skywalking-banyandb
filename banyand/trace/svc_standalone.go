...
// Service allows inspecting the trace elements.
...
func (s *standalone) PreRun(ctx context.Context) error {
	...
	if err = s.lfs.MkdirAll(s.snapshotDir, 0o755); err != nil {
		return err
	}
...
func (s *standalone) Trace(metadata *commonv1.Metadata) (Trace, error) {
	sm, err := s.schemaRepo.Trace(metadata)
	if err != nil {
		if errors.Is(err, resourceSchema.ErrGRPCResourceNotFound) {
			return nil, ErrTraceNotExist
		}
		return nil, err
	}
	return sm, nil
